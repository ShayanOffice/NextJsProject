...
export const RespTab = ({
  ...
  const submitFilter = async (e) => {
    ...
    try {
      ...
      req['Identity'] = filterIdentity; // اضافه کردن شناسه فیلتر که مربوط به کدام مدل است
      req && filter && (await filter(req).unwrap());
    } catch (err) {
  ...
  useEffect(async () => {
    try {
      ...
      toast.error(`درخواست مرتب سازی امکان پذیر نیست.${err}`);
    }
  }, [sort]);
        ...
        >
          <thead
            className='bg-white text-xs  lg:text-sm
                            drop-shadow-[2px_0_1px_rgba(29,38,109,0.3)]  transition-all duration-300
                            hover:drop-shadow-[0_2px_1px_rgba(29,38,109,0.3)] hover:transition-all hover:duration-300'
          >
            <tr>
                  ...
                  def && (
                    <td
                      ...
                      className={`${
                        sort[def.field] == 'asc'
                          ? 'bg-blue-200'
                          : sort[def.field] == 'desc' && 'bg-blue-400'
                      }`}
                      onClick={async () => {
                        !def.nonFilterable &&
                          setSort((prev) => {
                      ...
                      <td key={index}>
                        <form onSubmit={submitFilter}>
                          <input
                            ...
                            className={`w-full text-xs px-0.5 py-[0.07rem] text-gray-700 rounded appearance-none focus:outline-none focus:shadow-outline ${
